...
- 接続設定画面の実装ファイル追加
- 公開版 UI から接続設定導線を除外
- 任意 URL 保存による接続先切替の土台
- `Debug = development`、`Release = public` のビルド設定切替
...
2. カスタマイズ用ビルド構成の追加と運用を整理する
3. 出荷実績削除を Web API モードへそろえる
...
ただし、接続先は開発用 / 公開用 / カスタマイズ用の 3 モードで整理すること、接続設定はアプリ内画面として保持しつつ公開版では導線を持たせないこと、Azure デモ制御は初期版導入という方針は確定済みとする。
