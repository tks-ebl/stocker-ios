...
1. `StockerWebAPI/.env.local.example` を参考に、必要なら `StockerWebAPI/.env` を作成する
...
現時点のアプリ実装は PostgreSQL 前提です。

- `UseNpgsql(...)` を利用
- PostgreSQL 用 EF Core Provider を利用
- Migration も PostgreSQL 向け

そのため、Azure 本番 DB を `Azure SQL Database Serverless` にする場合は、アプリ側の DB 対応変更が別途必要です。

Azure 側の最小運用メモは以下を参照してください。

- `/docs/deploy-azure-minimal.md`
- `/docs/config.md`
- `/docs/operations.md`

既存の Azure 用テンプレートは以下に分離しています。

- `deploy/azure/.env.azure.example`
- `deploy/azure/containerapp.template.yaml`
- `deploy/azure/README.md`

ローカル Docker と Azure 本番を共存させる方針は次の通りです。

- Docker テスト用
  - `docker-compose.yml`
  - `.env.local.example`
  - `SEED__ENABLEDEMOSEED=true`
- Azure 本番用
  - `deploy/azure/*`
  - `SEED__ENABLEDEMOSEED=false`
  - DB 接続文字列と JWT 鍵は Secret 管理
